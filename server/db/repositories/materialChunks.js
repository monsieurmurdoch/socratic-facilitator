...
async function replaceForMaterial(materialId, sessionId, text, opts = {}) {
  ...
  if (chunks.length === 0) {
    // Still need to clear any existing chunks for this material.
    await clearByMaterial(materialId);
    return chunks;
  }

  // Embed BEFORE the transaction — Voyage may take seconds and we don't want
  // to hold a DB connection open across an external HTTP call.
  ...
  // Wrap DELETE + all INSERTs in a single transaction so partial failure
  // can never leave the material half-chunked or with mismatched embeddings.
